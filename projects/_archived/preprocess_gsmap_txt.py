...
def _get_filelist(timezone:str, target_day:datetime) -> list[str]:
    ...
    for i in range(0, 24):
        ...
        # f1 = glob(f"./data/gsmap/{d1.strftime('%Y/%m/%d')}/*{d1.strftime('%Y%m%d.%H%M')}*.dat.gz")
        # GPMMRG_MAP_2201010000_H_L3S_MCT_05A.txt.gz
        f1 = glob(f"{SOURCE}/{d1.strftime('%Y/%m/%d')}/*{d1.strftime('%y%m%d%H%M')}*.txt.gz")
...
def _aggregate(target_day:datetime, lock, destination:str, netcdf_filename:str) -> None:
    ...
    raw_data = _get_gsmap_txt(raw_files[0], column_value)

    # 0. Bound to the target area
    bound_lon = np.logical_and(np.array(raw_data[column_lon]) >= lon_range[0], np.array(raw_data[column_lon]) <= lon_range[1])
    bound_lat = np.logical_and(np.array(raw_data[column_lat]) >= lat_range[0], np.array(raw_data[column_lat]) <= lat_range[1])
    ...
    lats[:] = list(raw_data[column_lat][bound_lat])
    lons[:] = list(raw_data[column_lon][bound_lon])
    ...
    bounded = raw_data[bound_lat,bound_lon]
    ...
    for f in raw_files:
        ...
        # print(f)
        raw_data = _get_gsmap_txt(f, column_value)
        bounded = raw_data[bound_lat,bound_lon]
        temp = bounded.values.copy()
        if(sum(sum(temp < 0)) > 0): 
    ...
    data = data * 10
    data = np.where(np.isnan(data), 29999, data)
    # data[data < 0] = 29999
...
def preprocess() -> None:
    ...
    # list_of_rawdates:list[str] = glob(f"./data/gsmap/*/*/*")
    # # get a list of date that already preprocessd
    # list_of_preprocessd:set[str] = set()
...
if __name__ == "__main__":
    ...
    columns={config['gsmap']['columns']}
    source_path={config['gsmap']['source_path']}
    aggregated_path={config['gsmap']['aggregated_path']}
    geotiff_path={config['gsmap']['geotiff_path']}
    ...
    COLUMNS:dict = config['gsmap']['columns']
    BOUNDING_BOX = config['bounding_box']
    SOURCE = config['gsmap']['source_path']
    AGGREGATED_PATH = config['gsmap']['aggregated_path']
    GEOTIFF_PATH = config['gsmap']['geotiff_path']
    TIMEZONE = config['timezone']
    preprocess()